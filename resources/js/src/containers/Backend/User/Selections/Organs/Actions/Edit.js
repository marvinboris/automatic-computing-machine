...
class Edit extends Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        if (nextProps.organ && prevState.name === '') return updateObject(prevState, { ...nextProps.organ });
        return prevState;
    }
    ...
    render() {
        ...
        return <div>
            <Form id={id} onSubmit={this.submitHandler} className="row">
                <Input className="col-lg-12" type="text" name="name" placeholder="Nom de l'organe" onChange={this.inputChangedHandler} icon={faSignature} validation={{ required: true }} required value={name} />
...
const mapDispatchToProps = dispatch => ({
    submit: (id, data) => dispatch(actions.patchOrgansUpdate(id, data)),
});
