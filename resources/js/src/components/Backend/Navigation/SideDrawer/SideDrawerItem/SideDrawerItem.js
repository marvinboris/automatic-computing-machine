    ...
    if (!dropdown) content = <>
        ...
        const itemEls = items.map(({ link = '', text }) => (
            <li className="nav-item text-300" key={text}>
                <NavLink onClick={sideDrawerToggle} exact className="nav-link d-sm-none" to={path + link}><span className="position-relative" style={{ left: -8 }}>-</span> {text}</NavLink>
                <NavLink exact className="nav-link d-none d-sm-block" to={path + link}><span className="position-relative" style={{ left: -8 }}>-</span> {text}</NavLink>
            </li>
        ));
