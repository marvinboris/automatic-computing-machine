...
class Edit extends Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        if (nextProps.builder && prevState.name === '') return updateObject(prevState, { ...nextProps.builder });
        return prevState;
    }
    ...
    render() {
        ...
        return <div>
            <Form id={id} onSubmit={this.submitHandler} className="row">
                <Input className="col-lg-12" type="text" name="name" placeholder="Nom du constructeur" onChange={this.inputChangedHandler} icon={faSignature} validation={{ required: true }} required value={name} />
...
const mapDispatchToProps = dispatch => ({
    submit: (id, data) => dispatch(actions.patchBuildersUpdate(id, data)),
});
